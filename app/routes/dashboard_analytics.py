...
router = APIRouter(prefix="/analytics", tags=["Dashboard Analitik ve İstatistikler"])
...
def get_dashboard_statistics(
    ...
    """
    Dashboard için gerekli tüm metrikleri, grafik verilerini ve istatistikleri döner.
    """
    today = date.today()
    now = datetime.utcnow()
    ...
    # Yaklaşan toplantılar (Planlanan başlangıcı şimdiki zamandan büyük olanlar)
    ...
    # Ortalama toplantı süresi (Dakika cinsinden)
    avg_duration_query = db.query(func.avg(
        func.extract('epoch', Meeting.actual_end - Meeting.actual_start) / 60.0
    )).filter(Meeting.actual_end != None, Meeting.actual_start != None).scalar()
    
    avg_meeting_duration = round(avg_duration_query or 0, 1)

    # 2. Grafik 1: Aylara Göre Toplantı Dağılımı (Son 6 Ay)
    ...
    # 3. Grafik 2: Toplantı Türlerine Göre Dağılım
    type_counts = db.query(
        Meeting.meeting_type, func.count(Meeting.id)
    ).group_by(Meeting.meeting_type).all()
    
    type_distribution = [{"type": t[0].value if hasattr(t[0], 'value') else str(t[0]), "count": t[1]} for t in type_counts]

    # 4. En Fazla Toplantıya Katılan Kullanıcılar (Leaderboard)
    ...
    return {
        ...
        "charts": {
            "monthly_trend": monthly_distribution,
            "meeting_types": type_distribution
        },
